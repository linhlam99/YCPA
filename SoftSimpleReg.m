...
% epi{1}(125:150,55:95)= c_eps_0*11.3; % all permittivity is the same
epi{1}(100:125,35:65)= c_eps_0*11.3;
epi{1}(100:125,90:120)= c_eps_0*11.3;
...
f=250e12; %when increase the frequency the excitation is fast, the distance between each line gets smaller as well 
omega = f.*2.*pi;
betap = 0;
t0 = 30e-15;
% st = 15e-15;
st=-0.05; % set the "st" parameter to -0.05, the grating is more observable when increasing to new value
s = 0;
y0 = yMax/2;
sty = 1.5*lambda; 
bc{1}.s(1).paras = {mag,phi,omega,betap,t0,st,s,y0,sty,'s'}; %bc is used for set up boundary

Plot.y0 = round(y0/dx);

bc{1}.xm.type = 'a';
bc{1}.xp.type = 'e'; % when change to 'e' the signal is double and bounce
% back to the source while 'a' doesn't bounce back 
% bc{1}.xp.type = 'a';
bc{1}.ym.type = 'a';
bc{1}.yp.type = 'a';
